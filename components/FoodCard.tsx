...
const FoodCard: React.FC<FoodCardProps> = ({
    ...
    return (
        <div className="dn-box bg-box p-4 rounded-md shadow-md transition duration-500 ease-in-out transform hover:-translate-y-1 hover:shadow-lg relative">
            <h3 className="text-lg font-semibold mb-2 capitalize">{food.foodName}</h3>
            <p >Raw/Cooked: {food.rawOrCooked}</p>
            <p >Location: {food.location}</p>
            <p >Amount: {food.amount} kg</p>
            <p >
            ...
            <div className="mt-4 flex items-center">
                {isAddedToCart ? (
                    ...
                    <button
                        className="px-2 py-1 bg-green-500 rounded-md hover:bg-green-600 flex items-center"
                        onClick={onAddToCart}
                    >
